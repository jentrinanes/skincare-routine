...
const Settings = () => {
  ...
  const [profile, setProfile] = useState({ ...store.userProfile, ...store.user });
  ...
  const Section = ({ title, children }) => (
    <div className="space-y-3">
      ...
      <Card className="divide-y divide-stone-50 dark:divide-zinc-800">
        {children}
      </Card>
    </div>
  );
  ...
  const Toggle = ({ checked, onChange }) => (
    <button type="button" onClick={() => onChange(!checked)}
      className={`relative w-10 h-5.5 rounded-full transition-colors duration-200 ${checked ? 'bg-sage-500' : 'bg-stone-200 dark:bg-zinc-700'}`}
      style={{ height: '22px' }}>
  ...
  return (
    <div className="max-w-2xl space-y-8">
        ...
        <Card className="p-5">
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <FormField label="Skin type">
              <Select value={profile.skinType || 'Normal'} onChange={v => setP('skinType', v)}
            ...
            <FormField label="Timezone" hint="Used to determine today's date for your routine">
              ...
              >
                {(() => {
                  ...
                  return zones.map(z => <option key={z} value={z}>{z.replace(/_/g,' ')}</option>);
                })()}
              </select>
            </FormField>
      ...
      {/* Notifications */}
      <div className="space-y-3">
        <h3 className="text-xs font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-widest">Notifications</h3>
        <Card className="divide-y divide-stone-50 dark:divide-zinc-800">
          <SettingRow label="Routine reminders" description="Browser notifications for AM and PM routines">
            <Toggle
              checked={!!(store.userProfile?.notifications?.enabled)}
              onChange={async (v) => {
                ...
                dispatch({ type:'UPDATE_PROFILE', payload:{
                  ...store.userProfile,
                  notifications: { ...(store.userProfile?.notifications||{}), enabled: v }
                }});
              }} />
          </SettingRow>
          {store.userProfile?.notifications?.enabled && (
            <>
              <SettingRow label="AM reminder" description="Time for your morning routine reminder">
                <input type="time"
                  ...
                  value={store.userProfile?.notifications?.amTime || '07:30'}
                  onChange={e => dispatch({ type:'UPDATE_PROFILE', payload:{
                    ...store.userProfile,
                    notifications: { ...(store.userProfile?.notifications||{}), amTime: e.target.value }
                  }})} />
              </SettingRow>
              <SettingRow label="PM reminder" description="Time for your evening routine reminder">
                <input type="time"
                  className="px-3 py-1.5 text-sm rounded-xl border border-stone-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-stone-700 dark:text-stone-300 focus:outline-none focus:ring-2 focus:ring-sage-400"
                  value={store.userProfile?.notifications?.pmTime || '21:00'}
                  onChange={e => dispatch({ type:'UPDATE_PROFILE', payload:{
                    ...store.userProfile,
                    notifications: { ...(store.userProfile?.notifications||{}), pmTime: e.target.value }
                  }})} />
              </SettingRow>
      ...
      <Section title="Data">
        ...
        <SettingRow label="Clear daily logs" description="Remove all check-in history (keeps products & routine)">
          <Btn variant="outline" size="sm" onClick={handleClearLogs}>
            Clear logs
          </Btn>
        </SettingRow>
      </Section>
      ...
      <Section title="About">
        ...
        <SettingRow label="Barrier load scoring"
          description="Each active ingredient earns points based on its potential for irritation. Retinoids = 2–3, acids = 1–2, hydrators = 0.">
