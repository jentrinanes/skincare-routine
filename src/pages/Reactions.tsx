...
import { Icon, Btn, Card, FormField, EmptyState, PageHeader, ActivesBadges, ConfirmDialog, Modal } from '../components';
import { inputCls } from '../components/FormField';
import type { Reaction, Product } from '../types';

const BLANK: Omit<Reaction, 'id'> = {
  date: new Date().toISOString().split('T')[0]!,
  description: '',
  suspectedProducts: [],
};

function ReactionForm({ initial, products, onSave, onClose }: {
  initial: Reaction | null; products: Product[];
  onSave: (r: Reaction) => void; onClose: () => void;
}) {
  const [form, setForm] = useState<Omit<Reaction, 'id'>>({ ...BLANK, ...initial });
  const set = <K extends keyof typeof form>(k: K, v: (typeof form)[K]) => setForm(f => ({ ...f, [k]: v }));

  const toggleProduct = (id: string) => setForm(f => ({
    ...f,
...
export default function Reactions() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<Reaction | null>(null);

  const sorted = [...reactions].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

  const openAdd = () => { setEditing(null); setShowModal(true); };
  const openEdit = (r: Reaction) => { setEditing(r); setShowModal(true); };

  const handleSave = (reaction: Reaction) => {
    dispatch({ type: editing ? 'UPDATE_REACTION' : 'ADD_REACTION', payload: reaction });
  };
  ...
  return (
    <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {suspects.map(p => (
                              <div key={p.id} className="flex items-center gap-1.5 px-2.5 py-1 bg-terra-50 dark:bg-terra-900/20 rounded-lg">
                                ...
                                {p.actives.length > 0 && <><span className="text-xs text-terra-400 dark:text-terra-500">·</span><ActivesBadges actives={p.actives} max={2} /></>}
                              </div>
                            ))}
                          </div>
                        </div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editing ? 'Edit reaction' : 'Log reaction'} size="md">
