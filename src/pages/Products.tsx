...
import { Icon, Btn, Card, Badge, FormField, Select, EmptyState, PageHeader, ActivesBadges, StatusBadge, ConfirmDialog, Modal } from '../components';
import { inputCls } from '../components/FormField';
import { ACTIVES_LIST, PRODUCT_TYPES, PAO_OPTIONS, computeExpiry } from '../store/data';
...
function ActivesCombobox({ selected, onChange }: { selected: string[]; onChange: (v: string[]) => void }) {
  ...
  const removeActive = (a: string) => onChange(selected.filter(x => x !== a));
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Backspace' && !query && selected.length > 0) removeActive(selected[selected.length - 1]!);
...
function ProductForm({ initial, onSave, onClose }: { initial: Product | null; onSave: (p: Product) => void; onClose: () => void }) {
  const [form, setForm] = useState<Omit<Product, 'id'>>({ ...BLANK, ...initial });
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <FormField label="Status">
          <Select value={form.status} onChange={v => set('status', v as Product['status'])}
      ...
      {form.status === 'active' && (
        <div className="grid grid-cols-2 gap-3">
          <FormField label="Date opened">
            <input type="date" className={inputCls} value={form.openedDate ?? ''} onChange={e => set('openedDate', e.target.value || null)} />
          </FormField>
...
export default function Products() {
  ...
  const handleSave = (prod: Product) => {
    dispatch({ type: editing ? 'UPDATE_PRODUCT' : 'ADD_PRODUCT', payload: prod });
  };

  const daysUntilExpiry = (p: Product): number | null => {
  ...
  const FILTER_TABS = [['all', 'All'], ['active', 'Active'], ['unopened', 'Unopened'], ['finished', 'Finished']] as const;

  return (
    <div>
      <PageHeader
        title="Products"
        subtitle={`${products.filter(p => p.status === 'active').length} active · ${products.length} total`}
          ...
          {FILTER_TABS.map(([v, l]) => (
            <button key={v} onClick={() => setFilter(v)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                filter === v ? 'bg-sage-500 text-white' : 'bg-stone-100 dark:bg-zinc-800 text-stone-500 dark:text-stone-400 hover:bg-stone-200 dark:hover:bg-zinc-700'
              }`}>{l}</button>
          ))}
        </div>
      </div>
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-0.5">
                      ...
                      {expDays !== null && expDays <= 60 && expDays > 0 && <Badge color={expDays <= 14 ? 'terra' : 'yellow'}>{expDays}d left</Badge>}
                      {expDays !== null && expDays <= 0 && <Badge color="terra">Expired</Badge>}
                    </div>
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    <Btn variant="ghost" size="sm" onClick={() => openEdit(p)}><Icon name="edit" size={13} /></Btn>
                    <Btn variant="ghost" size="sm" onClick={() => setConfirmDelete(p)}><Icon name="trash" size={13} className="text-terra-400" /></Btn>
                  </div>
                </div>
              </Card>
            );
