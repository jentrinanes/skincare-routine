...
import { Icon, Card, PageHeader, ActivesBadges } from '../components';
...
export default function CalendarPage() {
  ...
  const getProduct = (log: Log): Product | null => {
    const ri = routine.find(r => r.id === log.routineItemId);
    if (!ri) return null;
    return products.find(p => p.id === ri.productId) ?? null;
  };
  ...
  const getDayData = (dateStr: string) => {
    ...
    return {
      am: completed.filter(l => l.period === 'AM').map(getProduct).filter(Boolean) as Product[],
      pm: completed.filter(l => l.period === 'PM').map(getProduct).filter(Boolean) as Product[],
      amSkipped: skipped.filter(l => l.period === 'AM').map(getProduct).filter(Boolean) as Product[],
      pmSkipped: skipped.filter(l => l.period === 'PM').map(getProduct).filter(Boolean) as Product[],
      total: completed.length,
    };
  };

  const getDayMood = (dateStr: string): 'empty' | 'light' | 'partial' | 'full' => {
    const data = getDayData(dateStr);
    if (data.total === 0) return 'empty';
    const totalPossible = routine.filter(r => r.period === 'AM').length + routine.filter(r => r.period === 'PM').length;
  ...
  const selData = getDayData(selected);
      ...
      <div className="grid lg:grid-cols-[1fr_300px] gap-5 items-start">
        <Card className="overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-stone-50 dark:border-zinc-800">
            ...
            <h2 className="text-sm font-semibold text-stone-800 dark:text-stone-100">{MONTHS[month]} {year}</h2>
          ...
          <div className="grid grid-cols-7 border-b border-stone-50 dark:border-zinc-800">
            {DAYS.map(d => (
              <div key={d} className="py-2.5 text-center text-xs font-medium text-stone-400 dark:text-stone-500">{d}</div>
            ))}
          </div>
            ...
            {cells.map((dateStr, i) => {
              ...
              return (
                <button key={dateStr} onClick={() => setSelected(dateStr)}
                  className={`relative aspect-square flex flex-col items-center justify-start pt-2 px-1 border-b border-r border-stone-50 dark:border-zinc-800/50 ${isLastCol ? 'border-r-0' : ''} ${isLastRow ? 'border-b-0' : ''} transition-colors group ${
                  ...
                  <span className={`text-xs font-semibold w-6 h-6 flex items-center justify-center rounded-full transition-colors ${
                    isToday ? 'bg-sage-500 text-white' : MOOD_TEXT[mood]
                  }`}>{dayNum}</span>

                  {data.total > 0 && (
                    <div className="flex gap-0.5 mt-1 flex-wrap justify-center">
                      {data.am.length > 0 && <div className="w-1.5 h-1.5 rounded-full bg-amber-400" />}
                      {data.pm.length > 0 && <div className="w-1.5 h-1.5 rounded-full bg-indigo-400" />}
                    </div>
                  )}

                  {mood !== 'empty' && (
                    <div className={`absolute bottom-0 left-1 right-1 h-0.5 rounded-full ${
                      mood === 'full' ? 'bg-sage-400' : mood === 'partial' ? 'bg-sage-300' : 'bg-sage-200 dark:bg-sage-800'
                    }`} />
                  )}
                </button>
              );
        ...
        <Card className="p-5">
          <p className="text-xs font-medium text-stone-400 dark:text-stone-500 uppercase tracking-widest mb-1">{selLabel}</p>
...
function ProductRow({ product, skipped }: { product: Product; skipped?: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 py-1.5 ${skipped ? 'opacity-50' : ''}`}>
      ...
      {(product.actives?.length ?? 0) > 0 && <ActivesBadges actives={product.actives} max={2} />}
    </div>
  );
}
